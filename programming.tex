\chapter{Hardware Description Languages}\label{kap:HDL}
...
HDLs differ from computer programming languages like C or Java in that case, that HDLs have possibilities to describe signal strength and propagation delays of signals. As CPLs have only sequential descriptions of algorithms, HDLs also model algorithms in concurrent matter. In HDLs two types of assignments exist, which are the blocking operator \textit{=} as well as the non-blocking assignment \textit{<=}.
